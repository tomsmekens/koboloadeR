      ...
      for (v in 1:nrow(chapters) )
      {
        ...
        crunch_question <- function(j)
        {
          ...
          if (questions.type == "select_one" ) {
            ...
            ## Remove when modality is NA
            data.selectmultilist <- as.data.frame(data.selectmultilist[,colSums(is.na(data.selectmultilist))<nrow(data.selectmultilist)])
            ...
            if (ncol(data.selectmultilist) %in% c("0","1")) {
              ...
              if (nrow(frequ) %in% c("0","1") |  nrow(meltdata) == 0) {
                ...
                cat(paste0("## Remove when modality is NA"),file = report.name.i.v.j ,sep = "\n", append = TRUE)
                cat(paste0("data.selectmultilist <- as.data.frame(data.selectmultilist[,colSums(is.na(data.selectmultilist))<nrow(data.selectmultilist)])"),file = report.name.i.v.j ,sep = "\n", append = TRUE)
                
                
                cat(paste0("totalanswer <- nrow(data.selectmultilist)"),file = report.name.i.v.j ,sep = "\n", append = TRUE)
                
                cat(paste0("## Remove the not replied to avoid going beyond 100%"),file = report.name.i.v.j ,sep = "\n", append = TRUE)
                ...
                if (nrow(disaggregation) == 0) {
                  ...
                  for (h in 1:nrow(disaggregation))
                  {
                    ...
                    if (disag.variable == questions.variable) {
                      ...
                      cat(paste0("totalanswer <- nrow(data.selectmultilist)"),file = report.name.i.v.j ,sep = "\n", append = TRUE)
                      cat(paste0("## Remove not replied to avoid going beyond 100%"),file = report.name.i.v.j ,sep = "\n", append = TRUE)
                      cat(paste0("data.selectmultilist <- data.selectmultilist[ data.selectmultilist[ ,1]!=\"Not replied\", ]"),file = report.name.i.v.j ,sep = "\n", append = TRUE)
                      cat(paste0("percentreponse <- paste0(round((nrow(data.selectmultilist)/totalanswer)*100,digits = 1),\"%\")"),file = report.name.i.v.j ,sep = "\n", append = TRUE)
